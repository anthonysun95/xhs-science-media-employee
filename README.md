...
这是一个可恢复、可迁移、面向普通读者的小红书科普生产技能包。领导只需要给出一个选题，并在研究完成后从 5 个标题中选择 1 个；数字员工会先明确读者最关心的问题、可执行结论和行动建议，再用支持证据与反证校准事实边界，继续完成文章写作、生成式封面、纯文字正文卡、逐页 PNG、合并 PDF 和评论区钩子。
...
- 选定标题和完整小红书文章；
- 第一页为生成式图文封面：标题必须大而醒目，并与生成图片有机融合；
- 中间正文页只做清楚的文字说明，不使用复杂插图、图表或装饰性排版；
- HTML/CSS 静态截图网页；
